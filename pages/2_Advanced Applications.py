...
# ********************
def main():
    st.title("Putting it all together using methods scientists created")
    st.header("Head-Related Transfer Functions (HRTF)")
    st.markdown("Although 'Head-Related Transfer Functions' sound complicated, and they are complicated to understand, the idea behind them is quite simple. You would have noticed when previously changing the delay, volume, and frequency that no matter how much you played around it was difficult to make a sound appear to come from a specific direction. That's because although those three factors significantly influence our ability to detect sound, the real world is complicated and these are not the only factors at play. The shape of your ear, sound bouncing off your body, and even the elevation the sound comes from can change our perception of a sound's origin. Below we show that scientist's have developed a method (ie a Head Related Transfer Function) that lets you change sound so it appears to come from another direction by changing only the angle and elevation.",unsafe_allow_html=True)
    st.header("Have a play!")
